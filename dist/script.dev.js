...
function startQuiz() {
  ...
  quizContainer.setAttribute("class", "container"); //allocate 15 seconds per question 5 questions = 75 seconds

  timeRemaining = quizQuestions * 15; //Setting the value of an attribute on the specified element. https://developer.mozilla.org/en-US/docs/Web/API/Element/setAttribute

  timeRemainingEl.setAttribute("value", timeRemaining);
  var quizQuestionInterval = setInterval(function () {
    if (timeRemaining < 1) {
      clearInterval(quizQuestionInterval); //Setting the value of attribute quizContainer the specified element qualified name: "class" with a value "container d-none" .
    ...
    timeRemaining--; //setting the time remaining to qualified name value
...
function displayQuestion(questionNumber) {
  ...
  var col = null;
  var row = null; //getting and setting attributes like a boss  then appending the question to row/column
  ...
  for (var i = 0; i < 4; i++) {
    ...
    col = document.createElement("div");
    col.setAttribute("class", "col-12");
    row.append(col); //https://www.youtube.com/watch?v=Pkf1m4rVZw0
